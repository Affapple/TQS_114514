...
public class EuromillionsDrawTest {
    ...
    @Test
    @DisplayName("Format shows all dips")
    public void testFormatDips() {
        String result = sampleCuppon.format();

        for (Dip dip : sampleCuppon) {
            assertTrue(result.contains(dip.format()), "Coupon doesnt contain dip: " + dip);
        }
    }
